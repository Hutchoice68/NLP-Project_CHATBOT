...
def call_ollama_llm(prompt, context=""):
    """เรียก LLM จาก Ollama ในเครื่อง (ตรวจสอบก่อนเรียกจริง)"""
    try:
        import subprocess

        # ตรวจสอบว่า Ollama server ทำงานอยู่หรือไม่
        result = subprocess.run(["pgrep", "-f", "ollama"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
            print("⚠️ Ollama server not running — skipping LLM response")
            return None
        ...
        print("❌ ไม่สามารถเชื่อมต่อ Ollama ได้ — ตรวจสอบว่า Ollama รันอยู่ที่พอร์ต 11434")
        return None
